...
O \emph{STMap}~\cite{pasqualin2020online} utiliza o mecanismo de sharing-aware mapping para aplicações STMs. Onde esse mecanismo fornece dados sobre a aplicação e arquitetura permitindo executar as threads concorrentes no mesmo nodo NUMA para otimizar a execução da aplicação.

Em tempo de execução o STMap coleta dados sobre os comportamentos compartilhados entre as threads, esses dados são usados para calcular um mapeamento otimizado de threads para núcleos e migra as threads em execução.
...
Para evitar overhead a coleta de dados da matriz de comunicação ocorre por amostragem, neste trabalho utilizamos 1 a cada 100 acessos por thread para acionar o contador, esta abordagem é baseada nos experimentos apresentados nos trabalhos~\cite{pasqualin2020online} e~\cite{pasqualin2020thread}. 
