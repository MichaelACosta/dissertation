...
% É uma matriz de endereços, mas o que tem nas linhas e colunas?
% É igual a matriz de comunicação?
% Esse array de estrutura chave valor é uma tabela hash?
% Se é, então usa o termo tabela hash.

\todo{aqui}
A matriz de endereços por sua vez armazena a informação de quais endereços foram acessados pelas threads. Quando duas threds acessam o mesmo endereço de memória, um evento é disparado armazenando o número de acesso por endereço na matriz. 

Para entender qual é o endereço mais acessado é utilizada uma tabela hash que possui uma estrutura de chave e valor. Esta estrutura utiliza como chave o endereço de memória acessado e como valor, a quantidade de acessos que este endereço recebeu.
...
\todo{arrumar}
% Nota que no teu algoritmo, primeiro tu ve se deve migrar e, caso deva migrar, tu escolhe uma fila. Nos dois parágrafos de texto tu inverte essa ordem, primeiro escolhendo a fila e depois decidindo se deve migrar (o que é estranho.)
% A: fala como sao usadas as matrizes para determinar a fila
A etapa de identificação das threads conflitantes busca entender a aplicação e a arquitetura para definir para qual fila a thread que gerou o abort deve ser migrada. Para isso a função~\emph{findBestQueue} recebe o identificador da thread atual e consulta na matriz de comunicação qual a outra thread em execução que possui mais acessos em comum à memória. Após identificar a outra thread, a função retorna a fila na qual a thread pertence. Após identificar a fila com maior afinidade, o LTMS utiliza uma das heurísticas para decidir se deve migrar a thread abortada ou não.
