...
\todo{entender o que é transação e o que é operação atomica (thread)}
...
Em transações longas, a política é baseada em grão fino para melhorar a precisão da predição e evitar o reinicio da transação. Se uma transação longa efetua um abort o escalonador marca a transação como Very Important Transaction (VIT) e copia seu conjunto de leitura para uma lista auxiliar global.

Com isto, para uma transação que realiza escritas efetue seu commit, ela verifica se o seu conjunto de escrita não conflita com os conjuntos de leituras das VITs que estão na fila global. Assim, o escalonador tenta garantir que as VITs não abortem novamente. Caso haja conflito, o commit é adiado por um tempo pré-determinado.

Em transações curtas a heurística evita a validação com base na intersecção para não adicionar overhead desnecessário. Sendo assim, ela apresenta uma ideia similar a do ATS, onde apresenta uma métrica de decisão para serializar as transações.

Para definir quando uma transação será serializada, o escalonador utiliza um valor calculado denominado Tempo Perdido (TP). Cada transação possui seu proprio TP, que é calculado com base em um valor pré-definido e a quantidade de reexecução da transação e seu TP anterior.

Toda transação que começa com TP igual a zero é executada livremente, conforme o TP aumenta o ProVit se encarrega de serializar as transações reduzindo a concorrencia, até o ponto em que somente uma transação poderá ser executada por vez.
