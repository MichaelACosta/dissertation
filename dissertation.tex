...
Para validação deste trabalho foi avaliado o desempenho da implmentação em arquitetura NUMA. Máquinas NUMA tem a vantagem de agregar maior paralelismo ao adicionar mais processadores sem aumentar o gargalo de acesso ao barramento. Sua principal caracteristica é o tempo não uniforme de acesso à memória. Nestes experimentos utilizamos o processador Intel Xeon E5-4650 com 96 núcleos e 192 threads em \emph{hyper threading}, os experimentos rodaram com cenarios de até 512 threads, sendo assim os testes com 256 e 512 threads ultrapassam o limite de threads em \emph{hyper threading} da máquina utilizada.

As figuras~\ref{temp} e~\ref{temp2} apresentam os resultados com tempo de execução em segundos para os benchmarks testados utilizando todas configuraçoes heuriticas para os cenarios de 1 a 512 threads. Temos como base de comparação os resultados obtidos com a TinySTM. Para estes experimentos o LTMS apresentou na maioria dos casos um melhor tempo de execução quando comprado a TinySTM. O melhor ganho de desempenho obtivo foi para o experimento \todo{melhor teste, figura e valor em porcentagem} com ...

O experimento Bayes~\ref{Bayes} apresentou melhor tempo de execução para a TinySTM que o escalonador LSTM, \todo{falar sobre os valores}...
